  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="text-center mb-8">
        <h1 className="font-display font-bold text-3xl text-text-main">Help & Support</h1>
        <p className="text-text-muted mt-2">Find answers or reach out to our team</p>
      </div>
        ...
        {[
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-md text-sm font-medium transition-colors ${
              activeTab === tab.id ? 'bg-white text-primary shadow-sm' : 'text-text-muted hover:text-text-main'
            }`}
          >
          ...
          {FAQS.map((faq, i) => (
            <div key={i} className="card p-0 overflow-hidden">
              ...
              >
                ...
                {openFaq === i ? <ChevronUp className="h-4 w-4 flex-shrink-0 text-text-muted" /> : <ChevronDown className="h-4 w-4 flex-shrink-0 text-text-muted" />}
              </button>
              {openFaq === i && (
